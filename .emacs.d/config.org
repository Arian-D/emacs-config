...
** Lexical binding
I like my programs to be sane, so gotta disable this shit.
- [[https://www.emacswiki.org/emacs/LexicalBinding][Emacs wiki page for it]]
- [[https://www.gnu.org/software/emacs/manual/html_node/elisp/Lexical-Binding.html][Elisp docs for it]]
...
** Keys
What is EMACS without the /MACS/.
*** General
...
#+begin_src emacs-lisp
  (use-package evil-collection
    ;; :straight
    ;; (evil-collection :local-repo "~/src/all/evil-collection/")
    :after evil
...
#+begin_src emacs-lisp
  (use-package consult
    ...
    (when (executable-find "rga")
      (setq consult-ripgrep-args "rga --rga-adapters=+pdfpages,tesseract --null --line-buffered --color=never --max-columns=1000 --path-separator /   --smart-case --no-heading --line-number ."))
...
** Early init
- [[https://www.gnu.org/software/emacs/manual/html_node/emacs/Early-Init-File.html][The docs]]
- [[https://github.com/hlissner/doom-emacs/blob/master/early-init.el][Doom's early-init]]
...
#+begin_src emacs-lisp
  (use-package zen-mode
    :straight (zen-mode :host github :repo "aki237/zen-mode"))
#+end_src
...
** LSP
Eglot, which will soon become built-in.
#+begin_src emacs-lisp
  (use-package eglot
...
#+begin_src emacs-lisp
  (use-package rustic
    ...
    :general ("<leader> SPC" #' rustic-cargo-build))
#+end_src
...
#+begin_src emacs-lisp
  ...
  (use-package exercism-modern
    ...
    :straight (exercism-modern :host github :repo "elken/exercism-modern"))
#+end_src
** (J|T)S
...
A really nice web UI for org-roam.
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  (use-package bang
    :straight
    (bang :local-repo "~/src/emacs/bang.el")
    :general ("<leader> !" #'!))
#+end_src
** Readin'
*** Novel
...
#+begin_src emacs-lisp
  (straight-use-package
   '(evmm :type git :host github :repo "arian-d/evmm"))
#+end_src

#+begin_src emacs-lisp
  (use-package qemu
    :straight
    (qemu :local-repo "~/src/universe/qemu.el/"))
#+end_src
...
* My content
#+begin_src emacs-lisp
  (defun my/config (&optional config)
    "Edit my config"
    (interactive)
    (find-file (or config "~/.emacs.d/README.org")))


  (general-def "<leader> ." #'my/config)
