...
A live document that infects every machine I use.

* 🤔 Understanding this
** 🤓 Literate
This configuration is [[https://en.wikipedia.org/wiki/Literate_programming][literate]], as in, the file you're reading /is/ the
actual program. Instead of tangling, I load it from [[./init.el][the init file]]
...
** 📦 Straight
[[https://github.com/raxod502/straight.el][Straight]] is like (n|gu)ix, but elispy. The garbo downstairs is the
...
Default to straight as the default PM. package.el, my beloathed, is painful.
#+begin_src emacs-lisp
  (setopt straight-use-package-by-default t)
#+end_src

Note: There is also package.el's =vc=, but it doesn't make it as easy to
manage forks or contribute. [[https://github.com/progfolio/elpaca][Elpaca]] is another one that looks cool, but
I haven't had the time to move to it.
...
#+begin_src emacs-lisp
  (use-package vertico
    ...
    :custom
    (vertico-resize t)
    (vertico-count (/ (window-total-height) 5))
  ...
  (use-package consult
    ...
    (when (executable-find "rga")
      (setopt consult-ripgrep-args
              "rga --rga-adapters=+poppler  --null --line-buffered --color=never --max-columns=1000 --path-separator /   --smart-case --no-heading --line-number ."))
...
*** Embark
Embark is for actions in the completion buffer
#+begin_src emacs-lisp
  (use-package embark
    :bind
    (("C-." . embark-act)
...
#+begin_src emacs-lisp
  (defmacro $ (&rest tokens)
    ...
    (pcase-let* ((stringify (lambda (s)
      ...
      `(lambda ()
         ,(format "Runs the command `%s'" shell-command)
         (interactive)
         (start-process ,command
                        nil
                        "bash"            ; TODO: Dynamically select based on $SHELL
                        "-c"
                        ,shell-command))))
#+end_src
...
We are so back.
#+begin_src emacs-lisp
  (fset 'my/📸 ($ grim -g "$(slurp)" - | wl-copy))
  (fset 'my/ocr ($ grim -g "$(slurp)" -
                  | tesseract - -
                  | wl-copy))
  (fset 'my/🔒 ($ swaylock -c "000000"))
  ...
  (use-package ewm
    ...
    ;; Configure inputs
     ...
     ;; Configure passthru prefixes
     (ewm-intercept-prefixes
      '("C-x" "C-u" "C-h" "M-x" "s-h"
        "s-SPC" "s-<return>"
        "s-<AudioNext>" "s-<AudioPrev>"
        ("s-f" :fullscreen)
        ("<print>" :fullscreen)))
     (ewm-unfocused-alpha 0.95)
     :bind
     ("s-<return>" . ghostel)
     ("s-SPC" . consult-buffer)
     ("s-p" . my/📸)
     ("s-t" . my/ocr)
     ([remap ewm-lock-session] . my/🔒)
     ("<print>" . meow-keypad)
     ("s-<AudioNext>" . ewm-next-surface-buffer)
     ("s-<AudioPrev>" . ewm-prev-surface-buffer)
     ;; :hook
     ;; (after-init . my/configure-fonts)
     :config
     (ewm--send-intercept-keys)
     (when (executable-find "gentoo-pipewire-launcher")
       (funcall ($ gentoo-pipewire-launcher restart &))))
#+end_src
...
#+begin_src emacs-lisp
  (defun my/configure-fonts ()
    "Configure fonts"
    (when-let* ((font-picker (apply-partially #'seq-filter #'x-list-fonts))
...
#+begin_src emacs-lisp
  (use-package combobulate
    :disabled ;; https://github.com/mickeynp/combobulate/issues/121
...
** HCL
Using this while there's no feasible treesit-based version.
#+begin_src emacs-lisp
  (use-package terraform-mode
    :straight (:host github :repo "hcl-emacs/terraform-mode"))
#+end_src
...
#+begin_src emacs-lisp :tangle no :noweb-ref org-faces-config
  ...
  ;;; Emacs can't ligature. This is just a larp.
  (setq-default
  ...
  (use-package org-capture
    ...
    (org-capture-templates
          '(("t"
             "✅ Todo"
             entry
             (file "~/Org/nextcloud/todo.org") "* TODO %?\n%U\n %i%x\n"
             :empty-lines 1
             :jump-to-captured t)
            ("r"
             "📖 Read"
             entry
             (file+headline "~/Org/shared/∀.org" "Reading list") "** TODO %? :read:\n%U\n %i%x\n"
             :empty-lines 1)
...
#+begin_src emacs-lisp
  (use-package org-agenda
    ...
    (org-agenda-file-regexp (rx (: ".org" (? ".gpg") eol)))
    ;; Hide useless stuff
...
#+begin_src emacs-lisp
  (use-package org-roam
    ;; :vc (org-roam :url "https://github.com/org-roam/org-roam"
    ;;               :rev "v2.3.1")
    :straight (org-roam
               :host github
...
#+begin_src emacs-lisp
  (use-package jinx
    :straight (:host github :repo "minad/jinx" :branch "1.11")
    :disabled
...
#+begin_src emacs-lisp
    (defun my/gptel--notify-if-unfocused ()
      (unless (memq 'gptel-mode local-minor-modes)
        (notifications-notify
         :title "gptel finished"
         :body "done")))
    ...
    (use-package gptel
      ...
      (gptel-post-response-functions . my/gptel--notify-if-unfocused)
...
- [[info:rcirc][RcIRC info page]]
  #+begin_src emacs-lisp
    (use-package rcirc
      ...
      (rcirc-prompt "→ ")
      :config
      ;; Clean up buffers after QUITting
      (advice-add 'rcirc-cmd-quit
                  :after
                  (lambda (_ _ _)
  ...
  (use-package elfeed
    ...
    (elfeed-feeds
     '("https://arian-d.github.io/blog/index.xml"
       ...
       ("https://mariozechner.at/rss.xml" ai))))
#+end_src
** 📧 Mail
#+begin_src emacs-lisp
  (use-package gnus
...
#+begin_src emacs-lisp
  (use-package empv
    ...
    :bind
    ("C-<AudioPause>" . empv-playlist-select)
    ("C-<AudioPlay>" . empv-playlist-select)
    ("<AudioNext>" . empv-playlist-next)
    ("<AudioPrev>" . empv-playlist-prev)
    ("<AudioPause>" . empv-pause)
    ("<AudioPlay>" . empv-resume)         ; TODO: Run a dwim behavior where in case of absense search is brought up
    :custom
...
#+begin_src emacs-lisp
  ...
  (defun my/save-yt-audio (url)
    ...
    (pcase-let* ((📁 "~/🎵/∀/")
      ...
      (pcase 🎵
        (`(file ,_) (message "TODO: Make %s playable" file))
        (_
  ...
  (advice-add #'empv-play :before #'my/save-yt-audio)
#+end_src
...
#+begin_src emacs-lisp
  (when (executable-find "kokoro-tts")
    (defun speak-region (beg end)
      "Use `kokoro-tts' + `mpv' for TTSing the region (or buffer if
  region is inactive)"
...
#+begin_src emacs-lisp
  ...
  (let ((etc.el (expand-file-name "etc.el" user-emacs-directory)))
    (when (file-readable-p etc.el)
      (load-file etc.el)))
#+end_src
