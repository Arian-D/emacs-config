...
#+end_src
** 💌 Messaging
# TODO: Add a function to check which is running and make the
# <Messenger> key behave accordingly.
*** Matrix
Ement
#+begin_src emacs-lisp
  (use-package ement
...
*** IRC
- [[info:erc][ERC info page]]
#+begin_src emacs-lisp
  (use-package erc
    :straight (:type built-in)
    :general
    ("<Messenger>" #'erc-switch-to-buffer)
    (erc-mode-map
