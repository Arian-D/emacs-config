...
#+begin_src emacs-lisp
  ...
  (with-eval-after-load 'ewm
    ...
    ;; Lock screen on idle
    (setopt ewm-idle `(300 . "swaylock -c 000000"))
    ;; Reconfigure fonts, since `display-graphic-p' returns nil when ewm is launching
    (my/configure-fonts)
    ...
    (define-key ewm-mode-map (kbd "s-t") ($ grim -g "$(slurp)" -
                                            | tesseract - -
                                            | wl-copy))
    (define-key ewm-mode-map (kbd "s-l") ($ swaylock -c "000000"))
    (add-to-list 'ewm-intercept-prefixes "s-<AudioNext>")
    (define-key ewm-mode-map (kbd "s-<AudioNext>") #'ewm-next-surface-buffer)
    (add-to-list 'ewm-intercept-prefixes "s-<AudioPrev>")
    (define-key ewm-mode-map (kbd "s-<AudioPrev>") #'ewm-prev-surface-buffer))
#+end_src
